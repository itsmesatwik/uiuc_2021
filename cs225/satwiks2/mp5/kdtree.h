...
class KDTree
{
  ...
  public:
    ...
    Point<Dim>& selectKth(vector<Point<Dim>>&, int, int, int, int);

    void sort(vector<Point<Dim>>&);

    KDTreeNode* buildTree(vector<Point<Dim>>&, int, int, int);

    Point<Dim> findNearest(KDTreeNode*, const Point<Dim>, Point<Dim>&, double&, int) const;

    double calculateRadius(Point<Dim>, const Point<Dim>) const;

    double calcualteOverlap(Point<Dim>, Point<Dim>, int) const;
...
//      * @todo Add your helper functions here.
//      */
//      int partition(vector<Point<Dim>>& points, int low, int high, int dim);
//      Point<Dim> quick_select(vector<Point<Dim>>& points, int low, int high, int dim);
//      KDTreeNode* recurse_tree_build(vector<Point<Dim>> points, int low, int high, int dim);
// };
//
// #include "kdtree.hpp"
// #include "kdtree_extras.hpp"

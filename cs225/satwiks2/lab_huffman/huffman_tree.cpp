...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (auto elem : frequencies) {
      if (elem.getCharacter() != '\0') {
        TreeNode *t = new TreeNode(elem);
        singleQueue.push(t);
      }
      else {
        TreeNode *t = new TreeNode(elem);
    ...
    while (!(singleQueue.size() == 0 && mergeQueue.size() == 1)
        ...
        TreeNode *t3 = new TreeNode(t1->freq.getFrequency() + t2->freq.getFrequency());
        t3->left = t1;
        t3->right = t2;
     ...
     * to thTreeNode *t = new TreeNode(elem)e root and you're done!
     */
    ...
    while (bfile.hasBits()) {
         ...
         */
         ...
         if (current->freq.getCharacter() != '\0') {
           char ch = current->freq.getCharacter();
           ss << ch;
           current = root_;
         }
         if (bfile.getNextBit()) {
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
     ...
     */
     if (current->freq.getCharacter() != '\0') {
       ...
       bfile.writeBit(0);
       TreeNode *t1 = current->left;
       TreeNode *t2 = current->right;
       writeTree(t1, bfile);
       writeTree(t2, bfile);
     }
}

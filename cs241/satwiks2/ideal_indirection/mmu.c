...
page_table_entry *mmu_get_pte(mmu *this, uintptr_t virtual_address, size_t pid) {
  uint32_t pd_entry_num = virtual_address >> 22;
  page_directory_entry *pd_entry = &((this->page_directories[pid]->entries)[pd_entry_num]);

  page_table *p_table = (page_table*) (uintptr_t) (pd_entry->base_addr << 12);
  uint32_t pt_entry_num = (virtual_address >> 12) & 0x3FF;
  page_table_entry *pt_entry = &p_table->entries[pt_entry_num];
  return pt_entry;
}
...
void mmu_read_from_virtual_address(mmu *this, addr32 virtual_address,
    ...
    if (virtual_address == 0) {
      ...
      return;
    } else if (!address_in_segmentations(this->segmentations[pid], virtual_address)) {
        mmu_raise_segmentation_fault(this);
        return;
    } else if (this->curr_pid != pid) {
      this->curr_pid = pid;
      tlb_flush(&this->tlb);
    }

    page_table_entry *pt_entry = tlb_get_pte(&this->tlb, virtual_address >> 12);
    if (pt_entry != NULL && pt_entry->present == 1) {
      pt_entry->accessed = 1;
      memcpy(buffer, (void*) (uintptr_t) ((pt_entry->base_addr << 12) + (virtual_address & 0xFFF)), num_bytes);
    } else {
      ...
      if (pt_entry == NULL) {
        ...
        if(pd_entry->present == 0) {
            ...
            uint32_t frame = ask_kernel_for_frame((page_table_entry*) pd_entry);
            pd_entry->base_addr = frame >> 12;
            read_page_from_disk((page_table_entry*) pd_entry);
            pd_entry->present = 1;
        }

        pt_entry = mmu_get_pte(this, virtual_address, pid);
        tlb_add_pte(&this->tlb, virtual_address >> 12, pt_entry);
      }

      if (!pt_entry->present) {
        mmu_raise_page_fault(this);
        uint32_t frame = (uint32_t) ask_kernel_for_frame(pt_entry);
        pt_entry->base_addr = frame >> 12;
        read_page_from_disk(pt_entry);
        pt_entry->present = 1;
      }
...
void mmu_write_to_virtual_address(mmu *this, addr32 virtual_address, size_t pid,
    ...
    page_table_entry *pt_entry = tlb_get_pte(&this->tlb, virtual_address >> 12);
    if (!address_in_segmentations(this->segmentations[pid], virtual_address) || (pt_entry != NULL && !pt_entry->read_write)) {
      mmu_raise_segmentation_fault(this);
      return;
    }
    ...
    if (pt_entry != NULL && pt_entry->present == 1) {
      pt_entry->accessed = 1;
      pt_entry->dirty = 1;
      memcpy((void*) (uintptr_t) ((pt_entry->base_addr << 12) + (virtual_address & 0xFFF)), buffer, num_bytes);
    } else {

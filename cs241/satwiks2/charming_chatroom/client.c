...
//Code reference: Professor Angrave Lecture 26
int connect_to_server(const char *host, const char *port) {
 ...
 /*QUESTION 6*/
    int connected_info = getaddrinfo(host, port, &info, &result);
    if(connected_info != 0){
    fprintf(stderr, "getaddrinfor: %s\n", gai_strerror(connected_info));
    exit(1);
    }
 
 /*QUESTION 7*/
    int connected_status = connect(fd, result->ai_addr, result->ai_addrlen);

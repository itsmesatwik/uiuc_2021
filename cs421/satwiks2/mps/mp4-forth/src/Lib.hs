...
liftCompOp op (x:y:xs) = Just $ if (y `op` x) then (-1:xs) else (0:xs)
...
--- ### Printing the Stack
-- asked friend for help who took class previously
...
-- analoous to transForLoop
--- ### Conditionals
transConditionals :: Transition -> Transition -> (ForthState -> ForthState)
transConditionals t1 t2 (i:is, d, o) =
    if i /= 0 then
        t1(is, d, o)
    else
        t2(is, d, o)
...
cstackThen (("else", kelse):("if", kif):(c, kold):cstack) = 
    Just ((c, knew):cstack) where knew = (transConditionals kif kelse) . kold
cstackThen (("if", kif):(c, kold):cstack) = 
    Just ((c, knew):cstack) where knew = (transConditionals kif id) . kold
cstackThen _ = Nothing

-- heavily analogous to for loop with a few changes
--- ### Indefinite Loops
transIndefiniteLoop :: Transition -> (ForthState -> ForthState)
transIndefiniteLoop kloop (i:j:is, d, o) = 
    let (i':j', d', o') = kloop(i:j:is, d, o)
    in
      if i' /= 0 then 
          (j', d', o')
      else 
          transIndefiniteLoop kloop(j', d', o')
transIndefiniteLoop _ _ = underflow
...
cstackUntil (("begin", kloop):(c, kold):cstack) = 
    Just ((c, knew):cstack) where knew = (transIndefiniteLoop kloop) . kold

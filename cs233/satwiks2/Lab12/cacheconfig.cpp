...
: _size(size), _block_size(block_size), _associativity(associativity) {
  ...
  */
    double off_bits = log(block_size * 1.0) / log(2.0);
    _num_block_offset_bits = (uint32_t)((int)off_bits);
    double idx_bits = log(size / block_size / associativity)/log(2.0);
    _num_index_bits = (uint32_t)((int)idx_bits);

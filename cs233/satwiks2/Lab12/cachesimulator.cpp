...
Cache::Block* CacheSimulator::find_block(uint32_t address) const {
   ...
   */
    ...
    auto curr = _cache->get_blocks_in_set(index);
    auto tag = extract_tag(address, _cache->get_config());

    for (auto val : curr) {
        if ((val->is_valid()) && (val->get_tag() == tag)) {
            _hits++;
            return val;
        }
    }
...
Cache::Block* CacheSimulator::bring_block_into_cache(uint32_t address) const {
   ...
   */
    ...
    for (auto val : curr) {
        if (!val->is_valid()) {
            val->set_tag(tag);
            val->read_data_from_memory(_memory);
    ...
    for (auto val : curr) {
        if ((val->get_last_used_time()) < (lru->get_last_used_time())) {
            lru = val;
        }
    }
